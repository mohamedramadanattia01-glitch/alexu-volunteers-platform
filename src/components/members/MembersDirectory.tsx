...
import { 
  ...
  Upload, Trash2, ShieldAlert, UserPlus, Phone
} from 'lucide-react';
...
export const MembersDirectory: React.FC<MembersDirectoryProps> = ({
  ...
  const { members, committees, currentUser, isHighLeadership, deleteMember, showNotification } = useApp();
  ...
  const confirmDeleteMember = () => {
    ...
    showNotification('success', `تم حذف العضو ${memberToDelete.fullName} بنجاح`);
              ...
              <div className="flex items-start justify-between gap-2.5 mb-2.5">
                <div className="flex items-center gap-2.5">
                  <img 
                    ...
                    alt="" 
                    className="w-11 h-11 rounded-xl object-cover border border-blue-400/40 shadow-md"
                  />
                  <div>
      ...
      {memberToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="relative w-full max-w-sm bg-slate-900 rounded-2xl shadow-2xl border border-rose-900/50 p-5 space-y-4 text-center">
            ...
            <div>
              <h3 className="text-base font-bold text-white">تأكيد حذف العضو</h3>
              <p className="text-xs text-slate-400 mt-1">
                هل أنت متأكد من رغبتك في حذف العضو <strong className="text-white">{memberToDelete.fullName}</strong> من قاعدة بيانات الفريق؟
              </p>
            </div>
