...
export const DocumentsView: React.FC = () => {
  ...
  const filteredDocs = documents.filter(doc => {
    ...
    return matchesSearch && matchesCat && matchesComm;
  });
  ...
  const handleDownload = (doc: DocumentItem) => {
    if (doc.pdfBase64) {
      ...
      // Demo simulated download of PDF document
      const sampleContent = `%PDF-1.4\n1 0 obj\n<< /Title (${doc.title}) /Author (${doc.uploadedBy}) >>\nendobj\n`;
      ...
      <div className="glass-card p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-blue-500/30">
        <div>
          <div className="flex items-center gap-2 mb-1 justify-end sm:justify-start">
            <span className="px-2.5 py-0.5 rounded-full bg-blue-500/20 text-blue-400 text-xs font-semibold">
              المكتبة التنظيمية ومستودع الـ PDF
            </span>
          </div>
          <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
            <span>مستودع لوائح وملفات اللجان (Documents Hub)</span>
            <FolderGit2 className="w-5 h-5 text-blue-400" />
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            الأدلة التنظيمية، خطط الطوارئ، قوالب التصميم، ومواد التدريب المعتمدة بصيغة PDF
          </p>
        </div>
        ...
        {canManage && (
          ...
          >
            ...
            <span>إضافة وثيقة PDF جديدة</span>
          </button>
        )}
      </div>
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] font-mono text-rose-400 font-bold px-2 py-0.5 rounded bg-rose-950/80 border border-rose-500/30 flex items-center gap-1">
              ...
              {/* Card Footer with Download and Management Actions */}
